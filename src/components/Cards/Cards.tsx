...
const Cards: React.FC = () => {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-5">
      ...
      <h2 className="text-3xl font-manrope font-extrabold">Agricultura de otros productores</h2>
      <div className='flex justify-center items-center gap-2'>
        {cards.map((card, index) => (
          <img className='w-[300px] h-[500px] ' key={index} src={card.thumbnail} alt={`Card ${index}`} />
        ))}
      </div>
    </div>
  );
